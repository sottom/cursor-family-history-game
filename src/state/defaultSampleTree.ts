...
export function buildFourGenerationSampleState(): AppState {
  ...
  // Gen 1 — great-grandparents (4 couples)
  let o = createNewPerson({ fullName: 'Omar West', shortName: 'Omar West' })
  let p = createNewPerson({ fullName: 'Petra West', shortName: 'Petra West' })
  let q = createNewPerson({ fullName: 'Quinn Lee', shortName: 'Quinn Lee' })
  let r = createNewPerson({ fullName: 'Rhea Lee', shortName: 'Rhea Lee' })
  let s = createNewPerson({ fullName: 'Saul Chen', shortName: 'Saul Chen' })
  let t = createNewPerson({ fullName: 'Tessa Chen', shortName: 'Tessa Chen' })
  let u = createNewPerson({ fullName: 'Uri Park', shortName: 'Uri Park' })
  let v = createNewPerson({ fullName: 'Vera Park', shortName: 'Vera Park' })

  const m1950 = { dateISO: '1950-06-01', location: undefined as string | undefined }
  ...
  // Gen 2 — grandparents
  let frank = createNewPerson({ fullName: 'Frank West', shortName: 'Frank West' })
  let grace = createNewPerson({ fullName: 'Grace Lee', shortName: 'Grace Lee' })
  let henry = createNewPerson({ fullName: 'Henry Chen', shortName: 'Henry Chen' })
  let iris = createNewPerson({ fullName: 'Iris Park', shortName: 'Iris Park' })

  const m1975 = { dateISO: '1975-08-15', location: undefined as string | undefined }
  ;[frank, grace] = setMarriage(frank, grace, m1975)
  ;[henry, iris] = setMarriage(henry, iris, m1975)

  // Gen 3 — parents
  let dana = createNewPerson({ fullName: 'Dana West', shortName: 'Dana West' })
  let evan = createNewPerson({ fullName: 'Evan Chen', shortName: 'Evan Chen' })
  const m2000 = { dateISO: '2000-05-20', location: undefined as string | undefined }
